...
\texttt{Apache Kafka} has become the gold standard for astronomical alert brokering due to its scalability, fault tolerance, and capacity to handle large data volumes for a wide variety of production-grade software, in academia and industry. Optical alerts from surveys like ZTF and LSST are transmitted as Avro packets\footnote{\url{https://avro.apache.org}} over Kafka, which means that the ability to feed from \texttt{Kafka} topics (topics represent one data stream that a clien can read from) is absolutely required by any alert brokering software. Its ecosystem of libraries available for all major programming languages makes it extremely easy to the client to develop pipeline around it. For these reasons, also adopting \texttt{Kafka} as our downstream message sharing system ensures compatibility with existing downstream services, which are designed to consume alerts from \texttt{Kafka}. For each survey supported by our software, an associated kafka consumer has been developed to feed from the \texttt{avro}-formatted alerts. The consumers take advantage of \texttt{Kafka}'s partitioning feature (where one topic is broken down in N partitions that a client can read from independently, with multiple processes) to process any given survey's alert stream(s) in parallel, maximizing the input rate.
...
\myworries{maybe the section under that comment should move elsewhere, or be in some info-card like we did in the NSF WOU ZTF+LSST proposal, to highlight the capabilities of rusts and its relevance for this project?}

Instead of implementing an alert consumer software for every survey, we heavily rely on \texttt{rust}'s \texttt{Trait} system. \texttt{Trait} let a developer define a blueprint for \texttt{Struct}s to implement (rust's data structure equivalent to Python classes). Thereafter, functions can be written to not take specific \texttt{Struct}s as input, but anything that implements one or more \texttt{Trait}s. For example, we define the \texttt{AlertConsumer} trait with a method definition called \texttt{consume} (responsible of reading from kafka with survey-specific parameters). Then, the main function responsible for reading from \texttt{Kafka} specifies that it takes any \texttt{Struct} that implements the \texttt{AlertConsumer} trait as input. This ensures type-safety across a codebase, while minimizing duplication and without sacrificing performance. Also, when multiple \texttt{Struct}s implementing the same \texttt{Trait} have an identical implementation for a function, said function can simply be defined and implemented in the trait itself, to avoid re-defining it in any of the \texttt{Struct}s.
This \texttt{Trait}, \texttt{Struct}s, and generic functions pattern is repeated throughout the BOOM codebase, where all alert processors that share common operations between at least 2 surveys can be deduplicated. By ensuring that little to no code is duplicated from one survey to another, we can ensure BOOM's scability as more surveys are supported in the future.

\subsection{Job scheduling with \texttt{Valkey}}

Whereas \texttt{Apache Kafka} shines when it comes to reliably sharing messages at scale across the web, it isn't the most performant solution for inter-process communication (topics are stored on disk, which results in a throughput limited by the host's IO capabilities). Instead, we opted for \texttt{Valkey}, a high-performance in-memory datastore backed by the Linux Foundation. \texttt{Valkey} can be used for a variety of workflows, including caching and message queues. Unlike \texttt{Apache Kafka}, all data stored with \texttt{Valkey} lives in RAM, ensuring a much higher throughout than physically possible on HDDs or SSDs. Once data is read from the various survey's kafka topics, BOOM's kafka consumer store alerts to be processed in a \texttt{Valkey} list, a simple array from which processes on the same machine or over network can read from, one element at a time or in batches. When read from a list, messages are removed from it, and therefore only be retrieved by one process at a time. This is precisely what we need from BOOM, where alert processing is not performed by one but many processes, to parallelize over the streams. Also, BOOM does not rely on one process to ingest, enrich, and filter a stream one alert at a time but is composed of dedicated processes for each of these processing steps (this approach and technical choice is detailed in section X). There, \texttt{Valkey} is used to communicate data between various processes with high-throughput. However, memory usage becomes a concern when relying heavily on in-memory storage technologies. With this in mind, BOOM sets limits on how many \texttt{Apache avro} alert packets are stored in memory at once. Other message queues used in BOOM only share minimal amount of information (mostly pointers to database documents, and unique identifiers of alert packets), and do not have a noticable memory impact.

\subsection{Spatial query-ready database with \texttt{MongoDB}}

\texttt{MongoDB} has proven to be a highly effective choice for alert brokering, as demonstrated by its successful implementation in \texttt{Kowalski}. Its cross-language support, flexibility, and powerful query language make it well-suited for building complex filtering pipelines for transient alerts. Namely, the \texttt{aggregation pipeline} feature let's us defined not only complex queries but a pipeline of queries with multiple stages, as a cycle of filtering (\$match stage), computing (\$project, \$addField, \$lookup ... stages) steps, perfectly suited to implement astronomical alert filtering pipelines. \texttt{MongoDB} offers both performance and scalability, essential for handling large data volumes efficiently. While \texttt{PostgreSQL} was a potential alternative, it would have required schema enforcement, limiting adaptability in an alert stream where fields can vary significantly. Moreover, \texttt{PostgreSQL} would have necessitated database migrations whenever a new astronomical catalog was integrated for crossmatching, and does not offer native support for common spatial queries (e.g. cone searches). By contrast, \texttt{MongoDB}'s schema-free design allows \texttt{BOOM} to dynamically accommodate new data sources without modifying the database structure, and natively supports \texttt{GeoJSON} indexes for fast spatial queries, such as cone-searches or nearest neighbour searches without any client-side implementation or extensions required. Just like Kowalski, BOOM relies very heavily on MongoDB's support for cone-searches of alert streams with archival/static catalogs, and other alert streams.

\myworries{This section deserves more work, I think we could maybe give a little more technical details about what things look like, and maybe that's also the right place to describe how each survey has 3 catalogs: one for invidual candidates/alerts, one for images/cutouts associated to these alerts, and one for astronomical objects.}

\subsection{Parallelized and Distributed Alerts Processing}

It had become clear with \texttt{Kowalski} that making one process responsible for processing one alert at a time entirely (ingestion, enrichment, and filtering) does not allow us to scale optimally. When one of the processing steps becomes a bottleneck, the only solution with a single worker approach is to make more copies of the same worker, rather than only scale the processing step that requires more resources to keep up with the expected throughput. For example, with the advances of machine learning techniques yielding ever larger and complex models, adding more models to run on an alert stream requires more compute time not for every step of the processing but only to run these additional models. Therefore, the ability to only scale processes responsible of running models without also allocating resources to other processing steps which do not need them, is much more effective and scalable. Next, we will describe how the alert processing steps have been split in multiple "workers":

\subsubsection{Alert Ingestion Worker}

BOOM's first worker type is the \texttt{Alert Ingestion} worker. It feeds from the \texttt{Valkey} queue that has been populated with \texttt{Apache avro} alert packets by the consumer(s), and its main role is to ingest xmatch-enriched reformatted alerts to the \texttt{MongoDB} collections of a given survey. It processes one alert packet at a time (multiple worker of this type are spawned to handle the load), going through the following streams:
\begin{itemize}
\item Read the \texttt{Apache avro} byte data to deserialize into rust \texttt{Struct}s matching the schema provided by the survey emitting the alerts. Here, we really heavily on the \texttt{serde} and \texttt{apache-avro} rust structs, the former also let's us customize the deserialization logic to modify the alert data's schema of each survey, in an effort to reduce some of the differences between different surveys' schemas, and to address some of their inefficiencies.
\item We separate the candidate (metadata about the current detection), candidate identifier (referred to as candid, the unique identifier of the candidate), and object identifier (objectId, unique identifier of the associated object) from its cutout images and time-series.
\item The candidate, its identifier and associated object's identifier are stored in the alert collection (indexes by candidate id).
\item The science, reference, difference cutout images are stored in the cutout collection (also indexes by candidate id).
\item For new objects - identified as the objects for which no entry indexes on the associated objectId is found in the object-level table - we cross-match the candidate's position with a number of static/archival catalogs. Since a new objectId is generated when we get the first ever alert at a given RA/Dec (+/- some uncertainty varying based on a survey's hardware and pipeline), and thereafter alerts at the same position are associated to the same objectId, cross-matches with static catalogs only need to happen once for a given objectId. As the position and catalog are immutable, so are the results of a cross-match. Cross-matches happen directly in the database using its native support for cone searches, using a GeoJSON index. Here, the radius used is the maximum between the positional uncertainty of the alert survey and the positional uncertainty of the instrument that was used to build the static catalog.
\item Similarly, we cross-match every single alert with the object collection of other surveys supported by BOOM. While the position is still immutable for a given objectId, here the catalogs we cross-match against are dynamic, populated with new objects as the surveys generate alerts at new positions. The cross-match radius is defined as the maximum positional uncertainty between 2 surveys being cross-matched.
\item Last but not least, we either create or update the object collection. New objects get a new entry in the object collection indexed on their objectId, containing the time-series (previous candidates, non-detections, and forced-photometry), and cross-matches with archival and static catalogs. For existing objects, the various time-series get new elements appended, and cross-matches with dynamic catalogs are updated.
\end{itemize}

Once an alert has been ingested, its candid is pushed to another \texttt{Valkey} queue for the next worker to read from. Surveys that can be enriched with machine learning scores have their candids sent to a queue that will be read by the \texttt{ML worker} (see section Y), otherwise these are directly sent to the \texttt{Filter worker} (see section Z).

\subsubsection{ML Worker}

While there is no obvious advantage to ingesting alerts in bulk - other than reducing round trips to the database, which we will explore in future iterations of BOOM if the alert ingestion becomes a bottleneck - there is a clear advantage to running machine learning models over batches of inputs, as these can easily be parallelized by the various Machine Learning frameworks available to us, using hardware accelerators (.e.g GPUs, TPUs). So, a Machine Learning (ML) worker will not read only one candid from \texttt{Valkey} but will a batch with a set maximum size (e.g. 1000 at a time, if less than 1000 alerts are queueing for MLing, instead of blocking until 1000 are available the ML worker will simply read as many candids as available). Once the candid are retrieved, we use an aggregation pipeline to retrieve the candidate, associated images, and full light curve(s) from the database. The alert data is then converted into the expected format for each ML model BOOM supports (if multiple models expect the same input data, features are only prepared once to avoid duplicating work). At this time, BOOM runs all 5 ACAI classifiers (TODO: add reference), and BTSbot (TODO: add reference). These models have successfully been used in Kowalski for a number of programs, including fully-automated follow-up programs like BTSbot, and BTSbot-nearby.
While most popular Machine Learning framework have been designed in Python and therefore not usable as-is in Rust or other languages, there are a number of solutions to port Python-trained models over to a rust-based pipeline. One can choose between:
\begin{itemize}
...
\item Since we are relying on an "compartimentalized" architecture with a worker type dedicated to ML that interacts with redis and mongodb, both of which are well supported in Python, one could simply implement this worker directly in Python. In fact, BOOM's architecture has been designed to maximize flexibility and inter-operability between languages, and was initially designed with a Python-based ML worker. However, we later decided to opt for a fully rust-based approach as we successfully converted Kowalski's Python-trained model to a format suitable for running directly in rust (described below).
\item The \texttt{pyo3} crate allows for seemless integration of Python code in a rust runtime. Using this package, a developer can directly run Python code within a rust program. However, this did not prove to yield a significance performance improvement compared to the complexity added to the software. Also, lack of documentation and community-backed examples applied to machine learning with \texttt{pyo3} made us stear away from this option.
\item Last but not least, most standard machine learning implementations, regardless of the framework used, can be converted to a framework and language agnostic format called \texttt{ONNX} (TODO: add reference if any, otherwise link in footer: https://onnx.ai/). \texttt{ONNX} is an open-source format that defines a set of common operators, to represent models trained in most ML frameworks in a graph-like format. Community-driven Python packages for both PyTorch and Tensorflow allow users to convert their trained model to onnx format, which can then be loaded in all languages for which an \texttt{ONNX} runtime has been implemented. In Rust, the \texttt{ort} crate (https://ort.pyke.io/) lets us load any \texttt{ONNX} model, while enabling graph representation optimization (\texttt{ONNX}'s graph optimizer is able to remove unnecessary, redudant, or suboptimal operators/nodes generated when converting models to this format, often allowing for higher levels of optimizations than achievable when running inference in the original framework used to train the model) and hardware acceleration (\texttt{ort} supports CPU, GPU, and TPU as well as a variety of backends well-suited for specific hardware, e.g. NVIDIA's cuda or Apple's CoreML).
\end{itemize}

After experimenting with all 4 approaches, integrating \texttt{ort} in a rust-based program to run Python-trained models converted to \texttt{ONNX} yielded the best "performance vs. complexity" ratio. So far, all Kowalski models have been converted to \texttt{ONNX} and have been implemented in BOOM.
\myworries{Should take about CIDER if we have something that we can run soon enough}

\subsection{Filter Worker}

\myworries{TODO: write about the filter worker}
...
\myworries{To rewrite with the sections written above in mind}
