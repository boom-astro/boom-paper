...
Harnessing the performance of a \texttt{Rust}-based software stack relying on a non-relational \mongo database combined with a \texttt{Redis}/\valkey in-memory processing queue and a \kafka cluster for message sharing, we demonstrate feature parity with the existing ZTF system with a throughput $\sim \boomthroughputfactor \times$ higher.
...
We further outline the development plan for \babamul, the public version of \boom processing LSST alerts.
...
Next, we will describe exactly how the alert processing steps mentioned above have been split into multiple ``workers'', as illustrated in Figure 2.
...
\todo{Also there is no mention of the compute used by MongoDB or Valkey here. Please also use these docker stats that the benchmark pipeline used to check what these are (can also look at the peak and median/mean if you think that's interesting)}
...
\rednote{I'll try to run \boom on a raspberry PI today. Would be a nice way to support the argument made above, and show that even a small config can run \boom, only at reduced speeds. -> UPDATE: doesn't run easily on RPI as MongoDB's recent versions don't support these lower end arm64 chips. Will try again on a mac mini tomorrow}
...
% \todo{I talked with Xander: We'll use a better example, such as https://fritz.science/source/T202505231407305m004723. While it does have enough data in DECam to be discovered without ZTF, we can just say that "we use real DECam data, but simulate LSST's cadence". That way we can give an example that's faithful to what LSST will give us in production, but using real data and not just a simulated lightcurve. That will be more scientifically interesting than this broker-y example of a somewhat random transient.}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=6.5in]{figures/SN2025kwy.png}
    \caption{
