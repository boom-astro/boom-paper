...
This will be essential to readily identify, for example, fast transients within these streams. The relatively slow cadence of LSST and other optical surveys' limited depth  make such identification difficult. Thus, the rate of evolution of these phenomena cannot be precisely measured when alerts from these surveys' streams are used separately.
...
The \textit{candids} sent by the Enrichment worker to the Filter worker are read and filtered on in batches. Rather than filters sequentially on one alert at a time, each filter runs sequentially but on all the alerts at once. This dramatically saves on DB usage and reduces the time required to filter on alerts. Once all filters have run on a batch of \textit{candids}, we are left with a hashmap where the keys are \textit{candids} that have passed at least one filter, and the values are the list of filters ids that each \textit{candid} has passed, and annotations if any. Then, we query the database to retrieve all the relevant data products for the subset of alerts that have passed at least one filter, and build \texttt{BOOM}'s final output: the \texttt{Alert} struct. This struct is identical for all surveys, but populated with a custom logic for each. It contains:
\begin{itemize}
...
One night of ZTF alerts (2025-03-11, with $\sim 30k$ alerts total,
a number sufficient to measure throughput) was ingested,
...
process counts for comparison. For both brokers, the database is pre-loaded with the full object-level data products (lightcurves, cross-matches) for all objects that have been detected prior to 2025-03-11. This ensures that the throughput test is representative of what is expected in production, where most alerts are emitted for existing -- and often persistent -- objects that may have thousands of data points in their light curves.
The machine used for throughput testing has a 2.9 GHz AMD EPYC 7002 processor with 32 cores, 64 threads, and 128 GB of 2933 MHz DDR4 memory.
...
though since there are three different worker counts to vary there is some
tuning required to achieve optimal worker counts,
which is seen in the figure as a more jagged line compared to \kowalski's
smoother thread count to processing time relationship.
We also see an interesting plateau in \boom's performance from 15
up until 19 workers,
which is due to the optimal enrichment to filter/alert worker ratio being
approximately 1:3,
meaning enrichment workers can become a bottleneck if this ratio is not maintained.
This presents the opportunity for future work to implement a dynamic
scheduler that can automatically adjust the number of workers of each
type based on the current load and processing times,
to ensure optimal performance at all times without the need for manual tuning.
...
\texttt{BOOM}'s memory footprint is also much lower,
using a maximum of \boommaxmem~GB compared to \kowalski's \kowalskimaxmem~GB
for the case with 7 total threads. Note that this value is the memory used by the workers alone
and doesn't include that used by Kafka, MongoDB, or Valkey. In fact, the $\sim \boomthroughputfactor \times$ throughput factor reported in the introduction was calculated with this 7 thread configuration. However, it is obvious from Figure~\ref{fig:scaling} that the throughput factor is much larger when more threads are allocated, as \boom continues to scale more efficiently than \kowalski. In fact, BOOM appears to keep scaling linearly even at 25 threads -- the maximum total worker count used in this throughput test -- where \boom is able to process $\sim 2.5 \times$ the LSST alert rate.
As mentioned in section \ref{sec:enrichment}, current development efforts are now looking at the integration of new features to the enrichment worker (parametric lightcurve fitting, new ML models, ...), as well as improving the support for GPU-based compute. While these introduce additional compute, early results hint at $\sim 10 \times$ faster processing using hardware acceleration, effectively improving throughput.
