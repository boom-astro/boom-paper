...
BOOM's filter worker is the third and last worker type to process alerts before producing an output that other systems can read from. Just like the Alert worker sends candidate IDs for the Enrichment worker to process, the Enrichment worker then sends the same candidate ids to the filter worker. At initialization, the filter worker loads user-defined filters from the database. These filters are MongoDB aggregation pipeline composed of a succession of \$project and \$match stage to iteratively transform the data and filter on the data. User-defined filters, when  written by BOOM's users, assume that all data products: candidates, full lightcurves, archival and survey cross-matches, are available. However, since these data products are divided in 2 tables in our data model (alert/candidate collections, and object-level collections), we pre-pend user-defined filters with additional stages 
