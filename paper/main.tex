...
\todo[inline]{Talk about AppleCider and it's ONNX version}

\todo[inline]{Talk about the features we are computing in the enrichment worker, not just ML}
...
Just as the Alert worker sends candidate IDs to the Enrichment worker for it to process the associated alerts, the Enrichment worker then sends the same candidate IDs of the alerts it processed to the filter worker. These are read in batches, so that filters run on a batch of alerts rather than on one of them at a time. Just as it did for the ML worker, this saves on DB usage and overhead, reducing the time required to filter on alerts, as well as the load put on the database. Once all filters ran on a batch of candidate IDs, we are left with a hashmap, with the candidates that have passed at least one filter as keys, and the list of filters ids that each candidate have passed and their annotations if any as values. We then query the database to retrieve all the relevant data products for these alerts and build \texttt{BOOM}'s own \texttt{Alert} struct. This struct is identical for all surveys (but populated with a custom logic for each), and contains:
...
- a list of filters that they passed, defined by their IDs and an optional `annotations` field.
- a list of `archival-matches`, containing all cross-matches with archival catalogs, as performed by the `Alert` worker. Each cross-match entry is characterized by its catalog name, and all the fields that are relevant for this catalog.
- a list of `survey-matches`, containing all the cross-matches with other surveys processed by `BOOM`. These follow as similar schema as described above.
\todo{TODO: This is something to discuss about the software itself. Should the photometry of matching surveys be contained on the list of survey matches, or contatenated with the photometry that's at the top level of the `Alert` struct (what currently only contains the photometry of survey of the alert that passed the filter}
...
Rubin alert stream (10,000 alerts every 30 seconds) and beyond,
...
from~\cite{Bachant2025BoomCalkit}.

\todo[inline]{Need to archive benchmarking repo to cite.}
