...
Just as the Alert worker sends candidate IDs to the Enrichment worker for it to process the associated alerts, the Enrichment worker then sends the same candidate IDs of the alerts it processed to the filter worker. These are read in batches, so that filters run on a batch of alerts rather than on one of them at a time. Just as it did for the ML worker, this saves on DB usage and overhead, reducing the time required to filter on alerts, as well as the load put on the database. Once all filters have run on a batch of candidate IDs, we are left with a hashmap, with the candidates that have passed at least one filter as keys, and the list of filters ids that each candidate have passed and their annotations if any as values. We then query the database to retrieve all the relevant data products for these alerts and build \texttt{BOOM}'s own \texttt{Alert} struct. This struct is identical for all surveys (but populated with a custom logic for each), and contains:
\begin{itemize}
