...
def group_photometry(photometry_path):
    ...
    photometry = [p for p in photometry if p['mag'] is not None and p['magerr'] is not None]
    print(f"Reduced to {len(photometry)} photometry points after removing non-detections")
...
def plot_photometry(grouped, min_mjd, ids, at_name=None, export=None, out_dir="."):
    ...
    for (filter, origin, instrument), points in grouped.items():
        ...
        if instrument == "DECam":
            ...
            ax.errorbar(mjds[0:1], mags[0:1], yerr=magerrs[0:1], fmt=marker, label=label, linestyle='None', alpha=0.75, color=color, markersize=marker_size)
            if len(mjds) > 1:
                ax.errorbar(mjds[1:], mags[1:], yerr=magerrs[1:], fmt=marker, linestyle='None', alpha=0.4, color=color, markersize=(marker_size))
        else:
            ax.errorbar(mjds, mags, yerr=magerrs, fmt=marker, label=label, linestyle='None', alpha=0.75, color=color, markersize=marker_size)
...
plot_photometry(grouped_photometry, min_mjd, ids, at_name, export='pdf', out_dir="./paper/figures")
